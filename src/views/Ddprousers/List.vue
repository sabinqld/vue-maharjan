...
export default {
  ...
  computed: {
    ...mapState("Ddprousers", ["list"]),
    ...mapGetters("Ddprousers", ["byId", "isLoaded"]),
    ...mapFields("Ddprousers", ["id", "empId", "pref"]),
    allLoaded() {
      if (this.isLoaded) {
        this.$emit("hide-msg");
        return true;
  ...
  methods: {
    ...mapActions("Ddprousers", ["saveItem", "deleteItem"]),
    addItem() {
      this.$router.push({ name: "ddprousers-add", params: { id: null } });
    },
    refreshData() {
    ...
    saveUser() {
      this.saveItem({ id: this.edit.id, empId: this.edit.empId, pref: this.edit.pref })
        .catch((err) => {
          console.log(err);
          this.clear();
        })
    ...
    downloadFile() {
      ...
      link.setAttribute("download", "DDProUsers.csv");
  ...
  beforeMount() {
    this.$store.dispatch("Ddprousers/checkload");
  }
};
</script>
