...
export default {
  ...
  data: () => ({
    ...
    errorMessage: null,
    toolbar: {
      displayName: "RWM Qlik Dashboard Users - Add User",
      actions: [{ event: "refresh", icon: "mdi-refresh", hint: "Refresh" }]
    }
  }),
  computed: {
    ...mapState("Qlikaccessrwm", ["itemLoaded", "itemDirty"]),
  ...
  methods: {
    ...mapGetters("Qlikaccessrwm", ["isDirty"]),
    ...mapActions("Qlikaccessrwm", ["saveItem", "cancelItem", "checkload"]),
    ...
    save() {
      if (this.validate()) {
        this.saveItem({ id: null, empId: this.empId });
        this.$router.go(-1);
      }
    },
  ...
  beforeMount() {
    ...
    this.$store.dispatch("Qlikaccessrwm/createNew");
  },
