...
export default {
  ...
  data: () => ({
    ...
    toolbar: {
      displayName: "RWM Qlik Dashboard Users",
      actions: [
  ...
  computed: {
    ...mapState("Qlikaccessrwm", ["list"]),
    ...mapGetters("Qlikaccessrwm", ["byId", "isLoaded"]),
    ...mapFields("Qlikaccessrwm", ["empId"]),
    allLoaded() {
      if (this.isLoaded) {
        this.$emit("hide-msg");
        return true;
  ...
  methods: {
    ...mapActions("Qlikaccessrwm", ["saveItem", "deleteItem"]),
    addItem() {
      this.$router.push({ name: "qlikaccessrwm-add", params: { empId: null } });
    },
    refreshData() {
      this.$store.dispatch("Qlikaccessrwm/load");
    },
    clear() {
      this.dialog = false;
    ...
    downloadFile() {
      ...
      link.setAttribute("download", "Qlikaccessrwm.csv");
  ...
  beforeMount() {
    this.$store.dispatch("Qlikaccessrwm/checkload");
  }
};
</script>
