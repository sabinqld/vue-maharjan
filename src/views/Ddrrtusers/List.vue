...
export default {
  ...
  computed: {
    ...mapState("Ddrrtusers", ["list"]),
    ...mapGetters("Ddrrtusers", ["byId", "isLoaded"]),
    ...mapFields("Ddrrtusers", ["id", "PID"]),
    allLoaded() {
      if (this.isLoaded) {
        this.$emit("hide-msg");
        return true;
  ...
  methods: {
    ...mapActions("Ddrrtusers", ["saveItem", "deleteItem"]),
    addItem() {
      this.$router.push({ name: "ddrrtusers-add", params: { id: null } });
    },
    refreshData() {
      this.$store.dispatch("Ddrrtusers/load");
    },
    clear() {
      this.dialog = false;
      this.edit.id = null;
    ...
    saveUser() {
      this.saveItem({ id: this.edit.id, PID: this.edit.PID })
        .catch((err) => {
          console.log(err);
          this.clear();
        })
    ...
    deleteSelectedUser() {
      //console.log("Delete method", this.edit.id);
      this.deleteItem({id: this.edit.id});
      this.dialog = false;
      this.deleting = false;
    },
    convertDate(date) {
    ...
    downloadFile() {
      ...
      link.setAttribute("download", "DDRRTUsers.csv");
  ...
  beforeMount() {
    this.$store.dispatch("Ddrrtusers/checkload");
  }
};
</script>
