...
export default {
  ...
  data: () => ({
    ...
    errorMessage: null,
    toolbar: {
      displayName: "DD RRT - Add User",
      actions: [{ event: "refresh", icon: "mdi-refresh", hint: "Refresh" }]
    }
  }),
  computed: {
    ...mapState("Ddrrtusers", ["itemLoaded", "itemDirty"]),
    ...mapState("Lookup", ["empLookup", "empStatus"]),
    ...mapFields("Ddrrtusers", ["empId", "pref"]),
  ...
  methods: {
    ...mapGetters("Ddrrtusers", ["isDirty"]),
    ...mapActions("Ddrrtusers", ["saveItem", "cancelItem", "checkload"]),
    ...
    save() {
      if (this.validate()) {
        this.saveItem({ id: null, PID: this.empId });
        this.$router.go(-1);
      }
    },
    ...
    searchEmp() {
      this.getEmp(this.tempEmpId)
        .then(() => {
          ...
          if (this.empLookup) {
            //console.log("Lookup:", this.empLookup);
  ...
  beforeMount() {
    ...
    this.$store.dispatch("Ddrrtusers/createNew");
  },
